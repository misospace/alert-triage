...
// the declared component paths, so the result must still be "touches". The
// component is declared relative to spec.path per Flux's contract, so
// "components/backup" resolves to "apps/payments/components/backup".
func TestClassifyCommitComponentPathMatch(t *testing.T) {
	srv := newFakeCommitServer(t)
	srv.payloads["0123456789abcdef0123456789abcdef01234567"] = commitPayload{
		files: []map[string]string{
			{"filename": "apps/payments/components/backup/job.yaml"},
		},
	...
	if len(rel.ComponentPaths) != 1 || rel.ComponentPaths[0] != "apps/payments/components/backup" {
		t.Fatalf("component paths = %v, want [apps/payments/components/backup]", rel.ComponentPaths)
	}
	if len(rel.MatchingPaths) != 1 || rel.MatchingPaths[0] != "apps/payments/components/backup/job.yaml" {
		t.Fatalf("matching paths = %v, want [apps/payments/components/backup/job.yaml]", rel.MatchingPaths)
	}
}
...
// TestClassifyCommitComponentRelativeDoesNotTouch is the inverse regression
// for the spec.path contract: with workload "apps/payments" and component
// "deploy/prod" (which resolves to "apps/payments/deploy/prod"), a commit
// that only touches "deploy/prod" at the repo root must NOT be classified
// as touching the workload. The old code, which treated traversal-free
// components as repo-root paths, would have matched the un-resolved path
// and produced a confident false "touches".
func TestClassifyCommitComponentRelativeDoesNotTouch(t *testing.T) {
	srv := newFakeCommitServer(t)
	srv.payloads["0123456789abcdef0123456789abcdef01234567"] = commitPayload{
		files: []map[string]string{
			{"filename": "deploy/prod/app.yaml"},
		},
		message: "touch only the repo-root path",
	}
	g := srv.client()
	rel := classifyCommit(context.Background(), g, "https://github.com/owner/repo",
		"refs/heads/main@sha1:0123456789abcdef0123456789abcdef01234567",
		"apps/payments", []string{"deploy/prod"})
	if rel.State != commitRelevanceDoesNotTouch {
		t.Fatalf("state = %q, want %q (reason=%q)", rel.State, commitRelevanceDoesNotTouch, rel.Reason)
	}
	if len(rel.MatchingPaths) != 0 {
...
func TestResolveWorkloadPaths(t *testing.T) {
	cases := []struct {
		...
		},
		{
			...
			// Flux documents spec.components as local, relative paths:
			// every component resolves against spec.path, even without "..".
			// "deploy/prod" under workload "apps/payments" is
			// "apps/payments/deploy/prod", NOT "deploy/prod".
			workload:   "apps/payments",
			components: []string{"deploy/prod"},
			want:       []string{"apps/payments/deploy/prod"},
			ok:         true,
		},
		{
			name:       "nested relative component with . segment",
			workload:   "apps/payments",
			components: []string{".//deploy//prod"},
			want:       []string{"apps/payments/deploy/prod"},
			ok:         true,
		...
		{
			name: "null byte in a plain component cannot traverse",
			// The null byte rides along inside the segment it is embedded in
			// and is inert: the component resolves against the workload path
			// like any other and the result simply never matches a real
			// changed file.
			workload:   "apps/payments",
			components: []string{"\x00deploy/prod"},
			want:       []string{"apps/payments/\x00deploy/prod"},
			ok:         true,
		},
		{
			name: "null byte in workload path is inert after join",
			// TrimSpace in cleanRepoPath strips the null byte from the
			// workload base; resolution then proceeds normally and nothing
			// can traverse.
			workload:   "apps/payments/\x00prod",
			components: []string{"../shared"},
			want:       []string{"apps/payments/shared"},
			ok:         true,
		},
